...
import { WebIntert, WebSubmit } from "~/shared/types/blog"
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readValidatedBody(event, body => WebSubmit.safeParseAsync(body))
  if (body.error) {
    const errors = body.error.errors.map(err => `${err.path.join('.')}: ${err.message}`).join('; ')

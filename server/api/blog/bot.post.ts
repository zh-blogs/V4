...
const handleDatabaseError = (e: DatabaseError, blog: { name: string; url: string }): InsertResult => {
  const errorDetails = {
    code: e.code,
    detail: e.detail,
  ...
  switch (e.code) {
    case PG_ERRORS.UNIQUE_VIOLATION:
      const field = e.constraint?.includes('url') ? 'URL' :
        e.constraint?.includes('name') ? '名称' : '记录'
      return {
        success: false,
        blog,
        error: `该${field}已存在`
...
export default defineEventHandler(async (event) => {
  ...
  const result = await readValidatedBody(event, body => BotUpdate.safeParseAsync(body))
  if (result.error) {
  ...
  for (const blog of insertBlogs) {
    ...
    try {
      await db.transaction(async (ts) => {
        const existing = await ts.selectDistinct()
          .from(Blogs)
          .where(or(
            eq(Blogs.name, blog.name), eq(Blogs.url, blog.url)
          ))

        if (existing.length > 0) {
          await ts.update(Blogs).set({
            ...blog
      ...
      if (e instanceof DatabaseError) {
        processResults.push(handleDatabaseError(e, { name: blog.name, url: blog.url }))
      } else {
        processResults.push({
          success: false,
          blog: { name: blog.name, url: blog.url },
